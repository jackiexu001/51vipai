...
它已经从首页内嵌的演示模块升级为独立页面。前端数据契约见 `etf/API.md`，默认读取：

```text
GET /api/v1/etf/dashboard
```

接口未部署或缺少来源、更新时间时，页面会显示不可用状态，不会使用演示行情代替真实数据。
...
这次 UI 不再追求“装饰感”，而是先建立一套稳定的设计系统：

```text
统一容器宽度：页面主体都放在同一个 1200px 容器里
统一间距节奏：主要使用 18px、24px、32px 这些固定间距
统一卡片样式：边框、圆角、阴影保持一致
统一字体层级：首页只有一个大标题，卡片标题和侧栏标题不再乱跳
统一占位状态：没有正文的内容显示为“规划中”，不再是空白
```

你以后判断一个页面是否专业，可以先看这 4 件事：

(() => {
  ...
  $("#portfolio-form").addEventListener("submit", async (event) => {
    ...
    try {
      ...
      result.innerHTML = `<div class="result-kpis"><article><span>最终资产</span><b>${money.format(data.finalValue)}</b><small>累计投入 ${money.format(data.invested)}</small></article><article><span>累计收益</span><b>${money.format(data.profit)}</b><small>${pct(data.returnPct)}</small></article><article><span>资金加权年化</span><b>${data.annualizedPct == null ? "—" : pct(data.annualizedPct)}</b><small>XIRR 口径</small></article><article><span>最大回撤</span><b>${pct(data.maxDrawdownPct)}</b><small>基准 ${data.benchmark} ${pct(data.benchmarkReturnPct)}</small></article></div>${chart(data.timeline, [{ key: "portfolio", label: "组合市值", color: "#c44d28" }, { key: "benchmark", label: data.benchmark, color: "#277c5d" }, { key: "invested", label: "累计投入", color: "#7c8490" }])}<p class="dca-disclaimer">行情来源：${escapeHtml(body.meta.source)} · 数据截至 ${escapeHtml(body.meta.asOf)}</p>`;
